...
with st.sidebar:
    ...
    mu_E = st.slider("Evidence strength (mu_E)", 0.5, 0.95, 0.65, 0.025)
    c_pen = st.slider("Asymmetric penalty (c_pen)", 1.0, 18.0, 6.0, 0.5)
    sigma_E = st.slider("Evidence noise (sigma_E)", 0.0, 0.3, 0.1, 0.01)
    sigma_expert = st.slider("Expert noise (sigma_expert)", 0.0, 0.5, 0.2, 0.01)
    sigma_peers_multiplier = st.slider("Peers noise multiplier", 1.0, 4.0, 2.0, 0.1)
    m_peers = st.slider("Number of peers (m_peers)", 1, 10, 3)
    f_peers = st.slider("Peers sampling fraction (f_peers)", 0.01, 0.5, 0.0625, 0.01)
    lr_base = st.slider("Learning rate (lr_base)", 0.01, 0.5, 0.1, 0.01)
    tau = st.slider("Decision temperature (tau)", 0.05, 1.0, 0.3, 0.05)
    w_init_expert = st.slider("Initial trust in Expert", 0.0, 1.0, 0.5, 0.05)
    w_init_peers = st.slider("Initial trust in Peers", 0.0, 1.0, 0.5, 0.05)
...
if run:
    ...
    st.plotly_chart(
        plotting.trajectory_figure(
            agg, "p_expert", title=f"P(Expert) · mu_E={mu_E} · c_pen={c_pen}"
        ),
        use_container_width=True,
    )
    st.plotly_chart(
        plotting.trajectory_figure(
            agg, "trust", title="Trust over trials"
        ),
        use_container_width=True,
    )
    st.plotly_chart(
        plotting.trajectory_figure(
            agg, "accuracy", title="Accuracy over trials"
        ),
        use_container_width=True,
    )
else:
